"""Parser for raw schema artifacts (v0.6 legacy, v0.7)."""
...
# v0.7 schema model
class RawSchemaV07(BaseModel):
    """v0.7 raw schema schema."""
...
class RawSchemaV06(BaseModel):
    """v0.6 legacy raw schema schema (missing schema_version)."""
    dataset: str
    record_count: int
    columns: List[Dict[str, Any]]
    source_file: Optional[str] = None
...
def parse_raw_schema(obj: Dict[str, Any], unknown_fields: str = "preserve") -> ParsedArtifact:
    """
    Parse raw schema with compatibility layer.
    
    Args:
        obj: Raw schema dict (from JSON)
    ...
    if found_version == "0.7":
        ...
        known_fields_v07: Set[str] = set(RawSchemaV07.model_fields.keys())
        ...
        if unknown_top_level:
            if unknown_fields == "reject":
                raise ValueError(f"Unknown top-level fields in v0.7 raw_schema: {unknown_top_level}")
        ...
        try:
            ...
            raise ValueError(f"Invalid v0.7 raw_schema structure: {e}")
    ...
    known_fields_v06: Set[str] = set(RawSchemaV06.model_fields.keys())
    ...
    if unknown_top_level:
        if unknown_fields == "reject":
            raise ValueError(f"Unknown top-level field in legacy raw_schema: {unknown_top_level}")
        # preserve mode: extract unknown fields
        for key in unknown_top_level:
    ...
    try:
        ...
        raise ValueError(f"Invalid legacy raw_schema structure: {e}")
